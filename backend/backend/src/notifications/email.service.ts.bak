...
export class EmailService {
  ...
  private getTemplateHtml(template: string, data: Record<string, any> = {}): string {
    const templates = {
      requestSubmitted: `
  ...
  async sendRequestNotification(type: 'submitted' | 'approved' | 'rejected', data: any): Promise<void> {
    const subjects = {
  ...
  async sendTransferNotification(data: any): Promise<void> {
    await this.sendEmail({
      to: data.email,
  ...
  async sendSubscriptionReminder(data: any): Promise<void> {
    await this.sendEmail({
      to: data.email,
